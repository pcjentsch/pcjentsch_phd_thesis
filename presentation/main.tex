...
\begin{frame}{First Project}
    \begin{itemize}
        \item Epidemiological landscape and population beliefs about the usage of non-pharmaceutical interventions (NPIs) help determine outcomes
        \item Population age distribution is a key factor in both NPI usage and COVID-19 mortality
        \item Model-based analyses were exploring which groups should be prioritized in vaccination\cite{bubar2020model,hoyt2020vaccine}.
    \end{itemize}
    Research question: how to prioritize vaccination rollout in a population, taking both age distribution and social dynamics into account? 
\end{frame}
\begin{frame}
...
\begin{frame}{Results}
            \begin{figure}
                \includegraphics[width = 0.75\textwidth]{covid/vaccination_by_mortality_small.pdf}
    ...
    \begin{columns}
        \begin{column}{0.6\textwidth}
            \begin{itemize}
                ...
                \item Evidence shows that movement of firewood is a major long distance vector
                
                \item Education and awareness is a major way we try to reduce this vector
            \end{itemize}


            Research question: how does educating a population on the issue compare to other methods of reducing the spread of pests ?
...
\begin{frame}{Outline of project}
    \begin{enumerate}
    \item Adapt a model such as Barlow et al. (CITE) to a larger, more realistic network
...
\begin{frame}{Data}
    \begin{columns}
        ...
        \begin{column}{0.55\textwidth}
            \begin{figure}
                \includegraphics[width=\textwidth]{firewood/core_graph_plot.png}
            \end{figure}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Chapter 3}
    \begin{itemize}
        \item Wildfire and bark beetles are disturbances integral to coniferous forest ecosystems in the western cordillera of North America
        \item Bark beetle outbreaks have always been destructive, but seem to be worse in recent decades
        \item Literature on causal relationship between bark beetle outbreaks and wildfire is extensive but inconclusive
        \item Existing modelling of these two coupled disturbances is sparse
    \end{itemize}
    Research question: Can stand structure explain key relationships between these two disturbances?
\end{frame}

\begin{frame}{Project Outline}

    \begin{itemize}
        \item Extend existing model of Duncan et al. to include wildfires 
        \item Explore parameter regime of extended model
        \item Introduce forest stand thinning procedures to reduce MPB outbreaks
        \item Show that these stand thinning procedures are able to work because of aforementioned heterogeneity 
    \end{itemize}
